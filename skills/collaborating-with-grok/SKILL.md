...
- **Use a denylist, not an allowlist.** On grok-build, a `--tools` allowlist that includes `web_search` fails at session build (`RequirementError`: the search agent needs `run_terminal_cmd` internally); the bridge warns if you try it. Removing shell/edit via `--disallowed-tools` keeps `web_search` working — verified: cited answer with `x.ai` source URLs in ~36s.
